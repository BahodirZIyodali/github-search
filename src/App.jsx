...
function App() {
  ...
  const searchUsers = async (searchUser)=>{
        axios.get(`https://api.github.com/users/${searchUser}`).then((res)=>{
        setUser(res.data)
       })  
    }  
    const [input, setInput] = useState('BahodirZIyodali')
  ...
  <div className="container mt-5 text-center card ">
    ...
    <input  value={input} onChange={(e)=> setInput(e.target.value)} type="text"   placeholder="Search or jump to..."  size="30" className=""/>
